...
\newcommand\qrr[1]{
  ...
  \ifx#1?\exists\mathrm R\else 
  \ifx#11\mathrm{ax}\else
  #1\mathrm R
  \fi\fi\fi\fi\fi
}
\newcommand\@qrr[1]{+_{#1}\mathrm R}
\newcommand\srr[1]{
...
\quad
	\infer[\!\!\scriptstyle{\qrr?}]{\seq A{\exists x.B}}{\seq A{B[t/x]}}
\quad
...
\begin{definition}
\defn{Strict coalescence} ($\scoal$) is the rewrite relation on pre-nets generated by the following rules, which apply symmetrically to the first formula in the sequent.
%
\renewcommand{\+}{{+}}
\renewcommand{\*}{{\times}}
\begin{align}
...
\tag{$\srr+2$}
\\[5pt]
	\net{\{\link AB,\link AC\}}A{B\*C}
& ~\scoal~ 
	\net{\{\link A{B\*C}\}}A{B\*C}
\tag{$\srr*$}
\\[5pt]	
	\net{\{\link AB\}}A{\exists x.B}
& ~\scoal~ 
	\net{\{\link[\sigma\!\minus\!x]A{\exists x.B}\}}A{\exists x.B} \quad \text{if }\sigma(x)\,\neq\,\bot
\tag{$\srr?$}
\\[5pt]
	\net{\{\link AB\}}A{\forall a.B}
& ~\scoal~ 
	\net{\link A{\forall a.B}}A{\forall a.B} \quad \text{if }a\notin\sigma
\tag{$\srr!$}
\end{align}
%
\defn{Unifying coalescence} ($\ucoal$) is the rewrite relation generated by the following rules:
\[
	(\urr+i)~,~(\urr?)~,~(\urr!)
	\quad
	...
	(\srr+i)~,~(\srr?)~,~(\srr!)
\]
except $\urr?$ does not require that $\sigma(x)$ is defined, plus the rule
\renewcommand{\+}{{+}}
\renewcommand{\*}{{\times}}
\begin{align}
...
\end{align}
%
A pre-net $\net\lambda AB$ \defn{coalesces}, in either relation $(\scoal)$ or $(\ucoal)$, if it coalesces to a single clean link $\net{(A,B)}AB$. It \defn{strongly coalesces} if every coalescence sequence can be extended to reach $\net{(A,B)}AB$.
\end{definition}

To be explicit, to generate coalescence as a rewrite relation means the following: a pre-net coalesces to another, $\net\lambda AB\scoal \net{\lambda'}AB$ if and only if there is a rewrite rule $\net\kappa CD\scoal\net{\kappa'} CD$ such that $C\leq A$, $D\leq B$, $\kappa\subseteq\lambda$, $\kappa'\subseteq\lambda'$, and $\lambda-\kappa=\lambda'-\kappa'$; and likewise for $(\ucoal)$.

\begin{definition}
An \all1 \defn{witnessing proof net} or \defn{witness net} is a pre-net $\net\lambda AB$ with $\lambda$ an axiom linking, that strict-coalesces.
\end{definition}
...
\[
\begin{array}{ccl}
	\Deseq{\infer{\seq P{\dual P}}{}}QR &=& \{\link QR\}
\\ \\
...
\begin{align}
	\net{\{\link AB,\link[\tau]AC\}}A{B\*C}
& ~\coal~ 
	\net{\{\link[\sigma\join\tau]A{B\*C}\}}A{B\*C}\quad\text{if }\sigma\comp\tau
\tag{$\*$}
\end{align}
