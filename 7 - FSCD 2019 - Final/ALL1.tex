...
\relatedversion{A full version \cite{Heijltjes-Hughes-Strassburger-2018} is available at \url{http://hal.inria.fr/hal-01867625/}}
...
\renewcommand{\*}{{\times}}
%
\defn{Strict sequentialization} ($\scoal$) is the rewrite relation on labelled pre-nets generated by the rules in Figure~\ref{fig:strict}, which replace one or two links by another in a pre-net $\net{\lambda_\Sigma^\Pi}AB$ where $B$ has a subformula $D_1\+D_2$, $D_1\*D_2$, $\exists x.D$, and $\forall x.D$ respectively. Symmetric cases are omitted.
%
...
\end{description}
\begin{center}\vspace{-.8ex}\(\quad\quad
\vc{
	\infer[\Qrr*]{\Seq{\exists x.A}{B\*C}}{
...
We would like to thank the anonymous referees for their constructive feedback. Dominic Hughes thanks his hosts, Wes Holliday and Dana Scott, at the UC Berkeley Logic Group. Willem Heijltjes was supported by EPSRC Project EP/R029121/1 \emph{Typed Lambda-Calculi with Sharing and Unsharing}. Lutz Stra{\ss}burger was supported by the ANR-FWF international
