...
\newcommandx\Wlink[7][1={},4=0,6=0]{
  \begin{scope}
    \coordinate (SRC) at ($(\@previousformulaname-#5.south) + (#4pt,0pt)$);
    \coordinate (TGT) at ($(\@formulaname-#7.north) + (#6pt,0pt)$);
    \draw[#1,cap=round] 
      	(SRC) 
    	--
      	node[color=black,anchor=east]{$\scriptstyle{#2}$}
      	node[color=black,anchor=west]{$\scriptstyle{#3}$} 
    	(TGT) ;
  \end{scope}
}
...
%	\net{\{~\link XY^\pi\comp\link[\tau]{\dual Y}Z^\phi~\mid~\link XY^\pi\in{\lambda_\Sigma^\Pi}~,~\link[\tau]{\dual Y}Z^\phi\in\kappa_\Theta^\Phi~\}~}AC
\]
