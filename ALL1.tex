...
In this paper we present proof nets for first-order additive linear logic (\all1). Our central challenge is to incorporate the essential content of first-order proof, the \defn{witness assignment} to existential quantifiers, into additive proof nets. In a standard approach, as in the sequent calculus rules below left, a witness to $\exists x.B$ is assigned by an immediate substitution $B[t/x]$. To assign different witnesses in different branches of a proof, the subformula may be duplicated first, giving $B[s/x]$ and $B[t/x]$, as below right.
%
\[
\hspace*{\fill}
\hspace*{-\parindent}
...
\defn{Witness nets} record witness assignment in substitution maps attached to each link. The example above left shows the proof net for the sequent proof above. (We will assume a different variable for each quantifier, and we attach links to predicate letters, as the root connective of an atomic proposition.) Witness nets are canonical for \all1 sequent calculus permutations. Composition is direct, where the witness assignments of links are composed through a simple process of \emph{interaction + hiding} similar to that of game semantics \cite{Mimram-2011}.

\defn{Unification nets} omit any witness information, as illustrated above right. In addition to canonicity, they embody a notion of \defn{generality}: where more than one witness could be assigned, unification nets do not require a definite choice, while witness nets do. Composition is direct, by straightforward relational composition. We compare further properties in Figure~\ref{fig:results}, in the conclusion, where we also discuss related work.
...
\subsection{Correctness: coalescence and slicing}
...
\hfill
\]
We extend slicing to witness nets, and coalescence to both witness nets and unification nets. Here, we illustrate the latter, and leave a discussion of slicing to the conclusion of the paper.
...
We have presented two notions of first-order additive proof net, witness nets and unification nets, to canonically capture the two natural notions of proof identity for first-order additive linear logic. The table in Figure~\ref{fig:results} summarizes the results of our paper, along with some observations we make below. 

Proof nets with additives and quantifiers existed before as \emph{monomial nets} \cite{Girard-1996}, where slices are managed explicitly with \emph{monomial weights}. These nets are not generally canonical: they admit the permutation (and duplication) of proof rules past implicit \emph{contractions}. However, it is possible that additive monomial nets could be restricted to some notion of canonical form. Likewise, coalescence (or contractibility) has not been studied for first-order monomial nets, though it has been extended to a related form of MALL proof nets \cite{Maieli-2007}.

Our slicing condition is closely related to a number of approaches to first-order classical logic. A formula $\exists x.A$ is interpreted as the sum (or classically, the \emph{disjunction}) over a fixed number of instantiations $A[t_1/x]\+\dots\+A[t_n/x]$. This can be traced back to Herbrand's Theorem \cite{Herbrand-1930}: $\exists x.A$ is equivalent to the infinite sum over $A[t/x]$ for all terms $t$ in the language, but for any given proof a finite set of terms suffices. \emph{Expansion tree proofs} \cite{Miller-1987,Heijltjes-2010} are a graphical proof formalism based on this idea.

In our slicing condition, the witnesses $t_1,\dots,t_n$ for the expansion $A[t_1/x]\+\dots\+A[t_n/x]$ are collected from the substitutions recorded at the links of the proof net. Expansion trees, in contrast, record the expansion and its terms explicitly. This could be an interesting alternative approach to additive proof nets, which we will explore in future work. It is expected, though, that this would sacrifice \emph{generality} and \emph{direct composition}.
