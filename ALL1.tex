...
\begin{proof}
By induction on the coalescence path.
\end{proof}

\newcounter{lemslicecorrect}
\setcounter{lemslicecorrect}{\thetheorem}
\begin{lemma}
\label{lem:slice-correct}
Strict coalescence preserves and reflects slice-correctness.
\end{lemma}
...
\newcounter{lemdependencycorrect}
\setcounter{lemdependencycorrect}{\thetheorem}
\begin{lemma}
\label{lem:dependency-correct}
Strict coalescence preserves and reflects dependency-correctness.
\end{lemma}
...
From left to right, let $\net{\lambda_\Sigma}AB$ be correct. By Lemma~\ref{lem:coalescence applies} either the net has coalesced, or a coalescence step applies. By Lemma~\ref{lem:slice-correct} and Lemma~\ref{lem:dependency-correct} the result of this coalescence step is again correct. It follows that the pre-net $\net{\lambda_\Sigma}AB$ strict-coalesces.
\end{proof}
...
\begin{proof}
By Theorem~\ref{thm:correctness} a correct pre-net coalesces, and by Lemma~\ref{lem:slice-correct} and Lemma~\ref{lem:dependency-correct} any coalescence step preserves correctness.
\end{proof}
...
\setcounter{theorem}{\thelemslicecorrect}
\begin{lemma}[Restatement]
Strict coalescence preserves and reflects slice-correctness.
\end{lemma}

\begin{proof}
For a strict coalescence step $L\scoal R$, we will show that the witness pre-net $L$ is slice-correct if and only if $R$ is. Let $L=\net{\lambda_\Sigma}AB$ and $R=\net{\kappa_\Theta}AB$. We will demonstrate that the left-hand side and right-hand side of each rule belong to the same slice of $\seq AB$, or in the case of $\srr?$, naturally corresponding slices.
...
\\
A slice $S$ of $\seq AB$ contains $\link C{D_1{\*}D_2}$ if and only if it contains either of $\link C{D_1}$ or $\link C{D_2}$, and may not contain both. Then $S\cap \lambda_\Sigma$ is a singleton if and only if $S\cap\kappa_\Theta$ is.
...
\setcounter{theorem}{\thelemdependencycorrect}
\begin{lemma}[Restatement]
Strict coalescence preserves and reflects dependency-correctness.
\end{lemma}

\begin{proof}
For a strict coalescence step $L\scoal R$, we will show that the witness pre-net $L$ is dependency-correct if and only if $R$ is. Again let $L=\net{\lambda_\Sigma}AB$ and $R=\net{\kappa_\Theta}AB$.

\begin{itemize}
	\item
$\link C{D_i}\scoal\link C{D_1{\+}D_2}$
\\
The column of $\link C{D_i}$ is that of $\link C{D_1{\+}D_2}$ plus $(D_i,\sigma\res{\ex[B]{D_i}})$ itself, which is minimal in the order $\dep$.
...
$\link CD\scoal\link[\sigma\minus x]C{\exists x.D}$
\\
The column of $\link CD$ is that of $\link[\sigma\minus x]C{\exists x.D}$ plus a pair $(D,\tau)$, which is minimal in $(\dep)$.

	\item
$\link CD\scoal\link C{\forall x.D}$
\\
The column of $\link CD$ is that of $\link C{\forall x.D}$ plus a pair $(D,\tau)$. The side-condition of the coalescence step is that $x\notin\sigma$; then $x$ does not occur free in any $(X,\rho)$, and $(D,\tau)$ is minimal in $(\dep)$.
\qedhere
\end{itemize}
\end{proof}
