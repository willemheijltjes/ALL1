\documentclass[UKenglish]{lipics-v2018}
...
\author
 ...
 {{http://www.lix.polytechnique.fr/Labo/Lutz.Strassburger/}} %{lutz.strassburger@inria.fr}
 {}
 {}
...
%
Witness nets are canonical for first-order additive sequent calculus. Unification nets in addition factor out any inessential choice for existential witnesses. Both notions of proof net are defined through coalescence, the additive version of multiplicative contractibility, and for witness nets an additional geometric correctness criterion is provided. Both feature a global composition operation.
\end{abstract}
...
Additive linear logic $(\all)$ is the logic of product $(\*)$, sum $(\+)$, and their canonical morphisms: projections, injections, and diagonals. It is naturally captured in \defn{proof nets} \cite{Girard-1987,Hughes-vanGlabbeek-2005}, which describe a morphism $A\to B$ by a sequent $\seq{\dual A}B$ plus a \defn{linking}, a relation between the propositional atoms of $\dual A$ and those of $B$.
%
Additive proof nets are \defn{canonical}: they factor out the permutations of sequent calculus, and correspond 1--1 to morphisms of the free category.
%
%Additive linear logic $(\all)$ is the logic of product $(\*)$, sum $(\+)$, and their canonical morphisms: projections, injections, and diagonals. It is naturally captured in \defn{proof nets} \cite{Girard-1987,Hughes-vanGlabbeek-2005}, which describe a morphism $A\mathbin{\shortrightarrow}B$ by a sequent $\seq{\dual A}B$ plus a \defn{linking}, a relation between subformulas in $\dual A$ and in $B$. Proof nets are \defn{canonical}, factoring out the permutations of sequent calculus.
%
%Additive linear logic $(\all)$ is the logic of product $(\*)$ and sum $(\+)$. Proofs, over a two-sided sequent $A\vdash B$ or, here, a one-sided sequent $\seq{\dual A}B$, are built from the canonical morphisms: projections, injections, and diagonals. These are captured naturally in \emph{proof nets} \cite{Girard-1987,Hughes-vanGlabbeek-2005}, which consist of a set of \emph{links} between subformulas in $\dual A$ and in $B$, as illustrated below left.
\[
...
\hfill
\]
\defn{Composition}, of proof nets over $\seq{\dual A}B$ and $\seq{\dual B}C$ to one over $\seq{\dual A}C$, is by the relational composition of their linkings along the dual formulas $B$ and $\dual B$, as illustrated above right.

Additive proof nets have two natural correctness criteria. \defn{Coalescence} \cite{Heijltjes-Hughes-2015,Hughes-Heijltjes-2016}, a counterpart to multiplicative \emph{contractibility} \cite{Danos-1990}, provides efficient correctness and sequentialization via local rewriting: it asks that the steps below left result in a single link, connecting both formulas. \defn{Slicing} \cite{Hughes-vanGlabbeek-2005} is a global, geometric criterion: it asks that each \defn{slice}, a choice to remove one subformula of each product along with all connected links, retains a single link.
%
\[
\hspace*{-\parindent}
\hspace*{\fill}
...
%In this paper we extend additive proof nets with first-order quantification. 
In this paper we present proof nets for first-order additive linear logic (\all1).
Our central challenge is to incorporate the essential content of first-order proof, the \defn{witness assignment} to existential quantifiers, into additive proof nets, while retaining their features and tools: canonicity, direct (relational) composition, coalescence, and slicing.

In a standard approach, as in the sequent calculus rules below left, a witness to $\exists x.B$ is assigned by an immediate substitution $B[t/x]$. To assign different witnesses in different slices, the subformula may be duplicated first, giving $B[s/x]$ and $B[t/x]$, as below right.
%
\[
\hspace*{\fill}
\hspace*{-\parindent}
...
\qquad
	\vc{\infer[\Qrr!]{\seq A{\forall x.B}}{\seq AB}}
\qquad\qquad
\vc{
...
\]
%
This is incompatible with a \defn{sequent + links} proof net design, where the conclusion sequent remains intact, and a subformula $B$ cannot be the subject of substitution or duplication. Instead, we propose two alternative treatments of witnessing terms, embodied in two notions of proof net: \defn{witness nets} and \defn{unification nets}.
% irrecoverably alters the subformula $B$ (since $B[t/x]$ may contain other occurrences of $t$, it is not possible to recover $B$)
%
Our solutions are based on the second author's recent \defn{unification nets} for first-order multiplicative linear logic \cite{Hughes-2018}. Their main feature is to omit existential witnesses altogether, and reconstruct them by unification.
%
\[
\hspace*{\fill}
\hspace*{-\parindent}
...
\defn{Witness nets} record witness assignment in substitution maps attached to each link. The example above left shows the net corresponding to the sequent proof above. (We will assume a different variable for each quantifier, and we attach links to predicate letters, as the root connective of a propositional atom.) Witness nets are canonical for \all1 sequent calculus permutations, and we give two correctness criteria, one via coalescence and one via slicing. Composition is direct, where the witness assignments of links must be composed, which is done through a simple process of \emph{interaction + hiding} similar to that of game semantics \cite{Mimram-2011}.

\defn{Unification nets} omit any witness information, as illustrated above right. In addition to canonicity, they embody a notion of \defn{generality}: where more than one witness could be assigned, unification nets do not require a definite choice, while witness nets do. Composition remains by relational composition, and correctness is by coalescence. During this process witnesses are reconstructed by first-order unification.
...
Coalescence~\cite{Heijltjes-Hughes-2015,Hughes-Heijltjes-2016}, like multiplicative contractibility, \cite{Danos-1990,Guerrini-Masini-2001} is effectively top-down sequentialization. Figure~\ref{fig:main example} illustrates the process. In the example, a link between subformulas $A$ and $B$ carrying a substitution $\sigma$ represents a sequent $\seq{A\sigma}{B\sigma}$. Each coalescence step ($\star S$) corresponds to a proof rule ($\star R$), by which the coalescence sequence generates the sequent proof from the axioms down to the conclusion.



In unification nets, existential witnesses are reconstructed during coalescence. The initial substitution map assigned to a link on atomic formulas $a=P(s_1,\dots,s_n)$ and $b=\dual P(t_1,\dots,t_n)$ is the most general unifier (\mgu) of $a$ and $\dual b$ (the first step in Figure~\ref{fig:unification example}). Coalescence then proceeds as for witness nets, except when joining two links from different slices with a product step (marked \hbox{\small$\;\urr*\;$} in Figure~\ref{fig:unification example}). Here, witness nets require both links to carry the same substitution, while for unification nets a common, more general map is generated by unification.\looseness=-1


%As illustrated by Herbrand's Theorem \cite{Herbrand-1930},



%\[
%\hspace*{\fill}%
%\hspace*{-\parindent}%\hspace*{8ex}% spacing hack to achieve central horizontal alignment
%\begin{tikzpicture}[net]
%	\formula[y=1.2]{\forall*{x.}\exists*{y.~}{\dual P}*{(y)}\+{\dual Q}*{(x,y)}}
%	\formula[y=0]{\exists*{z.~}P*{(f(z))}\*Q*{(z,t)}}
%	\Vlink[red,l,label={$\scriptstyle[f(x)/y\,,\,x/z]$~~}]{3,2}
...
{\all} is of course part of {\mall} (multiplicative-additive linear logic), and its lessons are clearly visible in the second author's canonical proof nets \cite{Hughes-vanGlabbeek-2005} for {\mall}, as well as the first and second author's locally canonical \emph{conflict nets}~\cite{Hughes-Heijltjes-2016}. Its proof nets also appear as the \emph{skew fibrations} in the second author's \emph{combinatorial proofs} for classical logic~\cite{Hughes-2006-proofs}, and in the third author's study of the \emph{medial rule} for classical logic~\cite{str:RTA07}. To prepare the ground for the cut elimination of first-order \emph{combinatorial proofs} is one motivation for the current presentation.



%Additive linear logic $(\all)$ is the logic of product $(\*)$ and sum $(\+)$, and their canonical morphisms: projections, injections, and diagonals. It is captured naturally in \defn{additive proof nets} \cite{Girard-1987,Hughes-vanGlabbeek-2005}, consisting of a two-sided sequent $A\vdash B$ or, here, a one-sided sequent $\seq{\dual A}B$ plus a \defn{linking}, a relation between subformulas in $\dual A$ and in $B$.
...
\begin{center}
\begin{tabular}{lcccc}
 &	Monomial nets & Expansion trees & Witness nets & Unification nets
\\\hline
   Canonicity & \xmark & \vmark & \vmark & \vmark
\\ Generality & \xmark &\xmark&\xmark& \vmark
\\ Direct composition &(\vmark) &\xmark& \vmark & \vmark
\\ Coalescence & ? & ? & \vmark & \vmark
\\ Slicing &\vmark &\xmark& \vmark &\xmark
\end{tabular}  
...
%\subsection{Alternative approaches: monomial nets and expansion trees}
We have presented two notions of first-order additive proof net, witness nets and unification nets, to canonically capture the two natural notions of proof identity for first-order additive linear logic.
%
Proof nets with additives and quantifiers existed before as \emph{monomial nets} \cite{Girard-1996}, where slices are managed explicitly with \emph{monomial weights}. These nets are not generally canonical: they admit the permutation (and duplication) of proof rules past implicit \emph{contractions}.
%
However, we do conjecture that additive monomial nets could be restricted to some notion of canonical form, which would be equivalent to the notion induced by \emph{expansion tree proofs}, provide another approach to canonicity for first-order quantification for classical logic \cite{Miller-1987,Heijltjes-2010}.
%
But the details of this relation are clearly beyond the scope of our paper.


%%\emph{Expansion trees} provide another approach to canonicity for first-order quantification for classical logic \cite{Miller-1987,Heijltjes-2010}.

In the formulation of expansion trees, a formula $\exists x.A$ is interpreted as the sum (or classically, the \emph{disjunction}) over a fixed number of instantiations $A[t_1/x]\+\dots\+A[t_n/x]$. This can be traced back to Herbrand's Theorem \cite{Herbrand-1930}: $\exists x.A$ is equivalent to the infinite sum over $A[t/x]$ for all terms $t$ in the language, but for any given proof a finite set of terms suffices.
%
We used this idea implicitly for our geometric correctness condition for witness nets (Section~\ref{sec:correctness}), which generalizes the existing \emph{slice-based} criterion for multiplicative-additive proof nets \cite{Hughes-vanGlabbeek-2005}. In constructing the slices over a formula, we interpreted $\exists x.A$ by an \emph{expansion} $A[t_1/x]\+\dots\+A[t_n/x]$, where the witnesses $t_1$ through $t_n$ are collected from the substitution maps of the links in the proof net.

However, for our definition of proof nets, we did not pursue this direction, and consider the choices made in this paper preferable. Witness nets and unification nets retain the original structure of \emph{formulas + links}, without requiring expansion trees. More importantly, as in the propositional case, they enable direct (non-inductive) composition by a relational composition of links.
%
%%Finally, available space does not permit a detailed exposition, which we defer to a journal version.
%
%We have presented two notions of first-order additive proof net, witness nets and unification nets, to canonically capture the two natural notions of proof identity for first-order additive linear logic.
%
The table in Figure~\ref{fig:results} summarizes the results of our paper and the observations in this section.
