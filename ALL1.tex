...
\\ \\
	...
	& \quad=\quad 
	  \Deseq{\deduce[\rule{0pt}{1pt}]{\seq AB}\pi}{A'}{B'}
\end{align*}
\lutz{why the $\Sigma$ in the third equation?}

\lutz{I don't see where the primes come in, i.e, in no equation there is a relation between $A$ and $A'$, etc.}
\end{definition}
...
\]
\lutz{I think a $\Xi$ in the index is just horrible.}
...
\lutz{shouldn't we say somewhere that the initial linking is just on atoms?}
\end{definition}
...
\begin{lemma}
Every pre-net on a coalescence path $\net{\lambda_\Sigma}AB \scoal^* \net{\{\link[\varnothing]AB^\pi\}}AB$ from a witness net satisfies the \defn{exact coverage} property: for any link $\link CD$, the domain of $\sigma$ is exactly the variables existentially quantified in $\seq AB$ thad bind \lutz{???} in $C$ and $D$,
\[
	\dom\sigma = \{\,x \mid C < \exists x.X \leq A\,\}
			\cup \{\,y \mid D < \exists y.Y \leq B\,\}~.
\]
...
\end{proof}
\lutz{I am missing something here. In the lemma there is only one statement. So, what is the first, and what the second?}
