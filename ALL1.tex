...
In this section we will describe the composition of two witness nets by a global operation. It consists of the relational composition of both linkings, as in the propositional case, where for each pair of links that are being connected, their witness maps are composed. Since each link corresponds to a slice, the operation is effectively first-order composition \cite{Mimram-2011} applied slice-wise. 

\[
\begin{tikzpicture}[net]
	\formula[right,y=1.2]{\exists v.A}
...
To compose two links $\link ab$ and $\link[\tau]{\dual b}c$, the substitutions on the  $\sigma_b$ and $\tau\subdual b$
