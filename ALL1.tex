...
For a link $\link CD$, a universally quantified variable in $C$ or $D$ does not occur free in the range of $\sigma$.
	...
	\item[Freshness]
For a link $\link CD$ in $\net{\lambda_\Sigma}AB$, variables in the range of $\sigma$ are fresh: distinct from any existentially quantified (as $\exists x$) variable in $\seq AB$.
\end{description}
...
\begin{definition}
 \defn{Strict sequentialization} ($\scoal$) is the rewrite relation on labelled pre-nets generated by the following rules. \defn{Strict coalescence} is the same relation ignoring proof labels, on witness pre-nets, illustrated in Figure~\ref{fig:coalescence}.
%
\renewcommand{\+}{{+}}
\renewcommand{\*}{{\times}}
\begin{equation}
