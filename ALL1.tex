...
\defn{Witness nets} record witness assignment in substitution maps attached to each link. The example above left shows the net corresponding to the sequent proof above. (We will assume a different variable for each quantifier, and we attach links to predicate letters, as the root connective of a propositional atom.) Witness nets are canonical for \all1 sequent calculus permutations, and we give both correctness criteria,  coalescence and slicing. Composition is direct, where the witness assignments of links must be composed, which is done through a simple process of \emph{interaction + hiding} similar to that of game semantics \cite{Mimram-2011}.

\defn{Unification nets} omit any witness information, as illustrated above right. In addition to canonicity, they embody a notion of \defn{generality}: where more than one witness could be assigned, unification nets do not require a definite choice, while witness nets do. Composition remains by relational composition, and correctness is by coalescence, during which process witnesses are reconstructed by first-order unification.

Coalescence~\cite{Heijltjes-Hughes-2015,Hughes-Heijltjes-2016}, like multiplicative contractibility, \cite{Danos-1990,Guerrini-Masini-2001} is effectively top-down sequentialization. Figure~\ref{fig:main example} illustrates the process. In the example, a link between subformulas $A$ and $B$ carrying a substitution map $\sigma$ represents a sequent $\seq{A\sigma}{B\sigma}$. Each coalescence step ($\star S$) corresponds to a proof rule ($\star R$), by which the coalescence sequence generates the sequent proof from the axioms down to the conclusion.
...
We have presented two notions of first-order additive proof net, witness nets and unification nets, to canonically capture the two natural notions of proof identity for first-order additive linear logic. We conclude with a quick table surveying our results.

\begin{center}
\begin{tabular}{lcc}
\vphantom)	& Witness nets & Unification nets
\\\hline
   Canonicity & \vmark & \vmark
\\ Generality && \vmark
\\ Direct composition & \vmark & \vmark
