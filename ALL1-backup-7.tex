...
\begin{definition}
 \defn{Strict sequentialization} ($\scoal$) is the rewrite relation on labelled pre-nets generated by the following rules, which apply symmetrically to the first formula in the sequent. \defn{Strict coalescence} is the same relation ignoring proof labels, on witness pre-nets.
%
\renewcommand{\+}{{+}}
\renewcommand{\*}{{\times}}
\begin{equation}
...
\noindent
A pre-net $\net{\lambda_\Sigma}AB$ \defn{strict-coalesces} if it reduces in $(\scoal)$ to a single link $\net{\{\link[\varnothing]AB\}}AB$.
\end{definition}


To be explicit, to generate coalescence as a rewrite relation means the following: a pre-net coalesces to another, $\net{\lambda_\Sigma} AB\scoal \net{{\lambda'_{\Sigma'}}}AB$ if and only if % there is a rewrite rule $\net{\kappa_\Xi} CD\scoal\net{\kappa'_{\Xi'}} CD$ such that $C\leq A$, $D\leq B$, ${\kappa_\Xi}\subseteq{\lambda_\Sigma}$, $\kappa'_{\Xi'}\subseteq{\lambda'_{\Sigma'}}$, and ${\lambda_\Sigma}-{\kappa_\Xi}={\lambda'_{\Sigma'}}-\kappa'_{\Xi'}$.
